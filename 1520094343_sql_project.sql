...
SELECT name, monthlymaintenance,
	...
	ELSE 'cheap' END AS facility_label
FROM Facilities
...
SELECT member_name, facility, cost FROM (
	SELECT CONCAT (firstname, ' ',  surname) AS member_name, Facilities.name AS facility,
		...
		FROM Members
			...
			INNER JOIN Facilities ON Bookings.facid = Facilities.facid
		WHERE
			Bookings.starttime >= '2012-09-14 00:00:00' AND	Bookings.starttime < '2012-09-15 00:00:00'
		) AS bookings
	WHERE cost > 30
ORDER BY cost DESC;
...
SELECT name, revenue FROM (
    ...
    FROM Bookings
    INNER JOIN Facilities ON Bookings.facid = Facilities.facid
    GROUP BY Facilities.name) AS allrvn WHERE revenue < 1000
ORDER BY revenue;         
